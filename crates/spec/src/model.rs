...
pub struct Spec {
    ...
    /// Files copied onto the root filesystem during the first boot.
    pub files: Vec<FileTransfer>,
    /// Commands run at the end of the first boot.
    pub run: Vec<RunStep>,
}
...
impl Spec {
    pub fn from_reader(reader: &mut Reader<'_>, ctx: &mut Context<'_>) -> Result<Self> {
        ...
        for mut entry in reader.table_list("files")? {
            files.extend(FileTransfer::read(&mut entry, ctx)?);
        }
        let mut run = Vec::new();
        for mut entry in reader.table_list("run")? {
            run.push(RunStep::read(&mut entry)?);
        }

        let spec = Spec { meta, system, user, ssh, network, hardware, provisioning, files, run };
    ...
    fn cross_validate(&self, ctx: &mut Context<'_>) -> Result<()> {
        ...
        let mut destinations: Vec<&str> =
            self.files.iter().map(|f| f.destination.as_str()).collect();
        destinations.sort_unstable();
        for pair in destinations.windows(2) {
            if pair[0] == pair[1] {
                return Err(Error::new(format!(
                    "`files`: `{}` is declared twice; a destination may only be written once",
                    pair[0]
                )));
            }
        }
        for file in &self.files {
            if let Some(reserved) = reserved_destination(&file.destination, &self.user.name) {
                return Err(Error::new(format!(
                    "`files`: `{}` is generated by rpi-provision itself ({reserved}); choose \
                     another destination",
                    file.destination
                )));
            }
        ...
        if total > LARGE_PAYLOAD_BYTES {
            ctx.warn(format!(
                "`files`: {total} bytes will be staged on the boot partition, which is small and \
                 FAT formatted. Consider fetching large files on the device instead."
            ));
        }
        Ok(())
    }
...
/// Roughly when a payload stops being a handful of configuration files.
const LARGE_PAYLOAD_BYTES: usize = 64 * 1024 * 1024;

/// Destinations the renderer writes itself.
///
/// Kept here rather than in `render` because validation has to happen before
/// rendering; `crates/render` has a test asserting that everything it
/// generates is covered by this list.
pub fn reserved_destination(destination: &str, user: &str) -> Option<&'static str> {
    const PREFIXES: [(&str, &str); 5] = [
        ("/etc/NetworkManager/system-connections/", "a NetworkManager profile"),
        ("/etc/ssh/sshd_config.d/10-rpi-provision.conf", "the sshd drop-in"),
        ("/etc/modules-load.d/rpi-provision-gadget.conf", "the gadget module list"),
        ("/etc/systemd/system/rpi-provision-gadget.service", "the gadget unit"),
        ("/usr/local/sbin/rpi-provision-gadget", "the gadget script"),
    ];
    for (prefix, what) in PREFIXES {
        if destination == prefix || (prefix.ends_with('/') && destination.starts_with(prefix)) {
            return Some(what);
        }
    }
    if destination == format!("/etc/sudoers.d/010-rpi-provision-{user}") {
        return Some("the sudo rule");
    }
    None
}
...
/// One file to copy onto the root filesystem during the first boot.
///
/// A `[[files]]` entry whose `source` is a directory expands into one of
/// these per file below it, so the rest of the pipeline never has to know
/// that directories were involved.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FileTransfer {
    /// As written in the specification, for diagnostics.
    pub source: String,
...
impl FileTransfer {
    fn read(reader: &mut Reader<'_>, ctx: &mut Context<'_>) -> Result<Vec<Self>> {
        let source = reader.req_string("source")?;
        let destination = reader.req_string("destination")?;
        let mode = reader.string_or("mode", "0644")?;
        let owner = reader.string_or("owner", "root")?;
        let group = reader.string_or("group", "root")?;
        reader.finish()?;

        validate_mode(&mode).map_err(|err| reader.key_error("mode", err.message))?;
        validate_device_path(&destination)
            .map_err(|err| reader.key_error("destination", err.message))?;
        for (key, name) in [("owner", &owner), ("group", &group)] {
            validate_username(name).map_err(|err| reader.key_error(key, err.message))?;
        }

        let full = ctx.base_dir.join(&source);
        let listed = ctx.provider.list_dir(&full).map_err(|err| {
            reader.key_error("source", format!("cannot read `{}`: {err}", full.display()))
        })?;

        let mut transfers = Vec::new();
        match listed {
            // A directory: every file below it, keeping the layout.
            Some(relatives) => {
                if relatives.is_empty() {
                    return Err(reader.key_error("source", format!("`{source}` is empty")));
                }
                for relative in relatives {
                    let Some(suffix) = relative.to_str() else {
                        return Err(reader.key_error(
                            "source",
                            format!("`{}` is not a UTF-8 path", relative.display()),
                        ));
                    };
                    let suffix = suffix.replace('\\', "/");
                    transfers.push(Self {
                        source: format!("{source}/{suffix}"),
                        destination: format!("{destination}/{suffix}"),
                        contents: read_asset(reader, ctx, &full.join(&relative))?,
                        mode: mode.clone(),
                        owner: owner.clone(),
                        group: group.clone(),
                    });
            ...
            None => transfers.push(Self {
                source: source.clone(),
                destination: destination.clone(),
                contents: read_asset(reader, ctx, &full)?,
                mode,
                owner,
                group,
            }),
...
fn read_asset(reader: &Reader<'_>, ctx: &Context<'_>, path: &Path) -> Result<Vec<u8>> {
    ctx.provider.read_bytes(path).map_err(|err| {
        reader.key_error("source", format!("cannot read `{}`: {err}", path.display()))
    })
}

/// A path on the device: absolute, and free of anything that would resolve
/// somewhere other than where it reads.
fn validate_device_path(path: &str) -> Result<()> {
    if !path.starts_with('/') {
        return Err(Error::new("must be an absolute path on the device, starting with `/`"));
    }
    if path.ends_with('/') {
        return Err(Error::new("must not end with `/`"));
    }
    if path.contains("//") {
        return Err(Error::new("must not contain an empty path component"));
    }
    for component in path.split('/').skip(1) {
        if component == "." || component == ".." {
            return Err(Error::new("must not contain `.` or `..`"));
        }
    }
    if path.contains(['\t', '\n', '\r']) {
        return Err(Error::new("must not contain a tab or a line break"));
    }
    Ok(())
}
...
fn validate_mode(mode: &str) -> Result<()> {
    ...
    {
        return Err(Error::new(format!(
            "`{mode}` is not an octal mode; write it as a quoted string such as \"0644\""
        )));
    }
    Ok(())
}

/// One command run at the end of the first boot.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RunStep {
    pub description: Option<String>,
    /// Emitted into the generated script as shell source, not as a value.
    pub command: String,
...
impl RunStep {
    fn read(reader: &mut Reader<'_>) -> Result<Self> {
        ...
        let command = reader.req_string("command")?;
        let ignore_failure = reader.bool_or("ignore_failure", false)?;
        reader.finish()?;

        if command.trim().is_empty() {
            return Err(reader.key_error("command", "must not be empty"));
        }
        // The command is interpreted by /bin/sh on the device, so it is the
        // one place a specification contributes shell source rather than a
        // value. Keeping it to a single line keeps the generated script
        // readable and its failure reporting per-command; anything longer
        // belongs in a script transferred with `[[files]]`.
        if command.contains(['\n', '\r']) {
            return Err(reader.key_error(
                "command",
                "must be a single line; transfer a script with `[[files]]` and run that instead",
            ));
        }
        if let Some(text) = &description {
            if text.contains(['\n', '\r']) {
                return Err(reader.key_error("description", "must be a single line"));
            }
        }
        Ok(Self { description, command, ignore_failure })
    }
}
